...
// Receives the submission from the form and emails it to the firm with the
// Word + PDF as real attachments, using Resend's HTTP API.
//
// Required environment variables (set in Netlify → Site configuration → Environment variables):
//   RESEND_API_KEY  — your Resend API key (starts with "re_")
//   TO_EMAIL        — the firm inbox that should receive intakes
// Optional:
//   FROM_EMAIL      — sender, e.g. "Del Toro Law Intake <intake@yourdomain.com>"
//                     Defaults to Resend's test sender, which can ONLY deliver
//                     to the Resend account owner's email until you verify a domain.

const RESEND_URL = "https://api.resend.com/emails";
const MAX_BODY_BYTES = 8 * 1024 * 1024; // 8 MB guard
